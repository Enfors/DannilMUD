...
**** Event activation
This is what happens when the activate method is called in an Event:

1. If broadcast is False:
   1. If event.target is set:
      1. Call target.recv_evt(evt)
*** Daemons
**** The update daemon
...
|-------------+------------------------------------|
| cmds/player | Commands available to all users    |
| cmds/wiz    | Commands available only to wizards |
| cmds/admin  | Commands available only to admins  | 
