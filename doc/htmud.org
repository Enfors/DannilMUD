...
remember, and make paths short. All lowercase letters.
...
"private" - meant to only be called from within the object - have a
name beginning with an underscore. Only other member functions in the
same class are allowed to call such functions.
